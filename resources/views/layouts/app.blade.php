...
<body class="  ">
    ...
    <aside class="sidebar sidebar-default sidebar-white sidebar-base navs-rounded-all ">
        <div class="sidebar-header d-flex align-items-center justify-content-start">
            <a href="" class="navbar-brand">
                ...
                <!--logo End-->
                <h4 class="logo-title">Mararena</h4>
            </a>
                ...
                <ul class="navbar-nav iq-main-menu" id="sidebar-menu">
                        ...
                        <ul class="sub-nav collapse" id="horizontal-menu" data-bs-parent="#sidebar-menu">
                            ...
                            <li class="nav-item">
                                <a class="nav-link " href="{{ route('sizes.create') }}">
                                    ...
                                    <span class="item-name"> Tallas </span>
                                </a>
                            </li>
                            <li class="nav-item">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="{{ route('sales.create') }}">
                            ...
                            <span class="item-name">Venta de ropa</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="{{ route('sales.restaurante') }}">
                            <i class="bi bi-cup-hot-fill"></i>
                            <span class="item-name">Cafetería</span>
                        </a>
                    </li>
                    <li class="nav-item">
                    ...
                    <li class="nav-item">
                        ...
                        <ul class="sub-nav collapse" id="sidebar-special" data-bs-parent="#sidebar-menu">
                            <li class="nav-item">
                                <a class="nav-link " href="{{ route('storages.index', ['tipo' => 'Ropa']) }}">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                                <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                                    ...
                                    <i class="sidenav-mini-icon"> R </i>
                                    <span class="item-name">Ropa</span>
                                </a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link " href="{{ route('storages.index', ['tipo' => 'Cafetería']) }}">
                                    <i class="icon">
                                        <svg class="icon-10" xmlns="http://www.w3.org/2000/svg" width="10" viewBox="0 0 24 24" fill="currentColor">
                                            <g>
                                                <circle cx="12" cy="12" r="8" fill="currentColor"></circle>
                                    ...
                                    <i class="sidenav-mini-icon"> CAF </i>
                                    <span class="item-name">Cafetería</span>
                                </a>
                            </li>
                        </ul>
                    ...
                    <li class="nav-item">
                        <a class="nav-link" aria-current="page" href="{{ route('attendance.create') }}">
        ...
        <div class="position-relative iq-banner">
            ...
            <nav class="nav navbar navbar-expand-lg navbar-light iq-navbar">
                <div class="container-fluid navbar-inner">
                    <a href="../dashboard/create.html" class="navbar-brand">
                        <!--Logo start-->
                        <!--logo End-->
    ...
    <!-- Wrapper End-->
    <!-- offcanvas start -->
    <div class="offcanvas offcanvas-end" tabcreate="-1" id="offcanvasExample" data-bs-scroll="true" data-bs-backdrop="true" aria-labelledby="offcanvasExampleLabel">
    ...
    <script>
        ...
        const ticketRawTemplateUrl = @json(route('sales.ticket_raw', ['sale' => '__SALE_ID__']));
        const qzDefaultPrinterName = @json(config('qz.printer_name', 'Ticketera'));
        const qzSignatureAlgorithm = @json(config('qz.signature_algorithm', 'SHA1'));
        const qzPrintMode = @json(config('qz.print_mode', 'pixel'));
        const qzProductionLock = @json((bool) config('qz.production_lock', true));
        const qzAllowedOrigins = @json(config('qz.allowed_origins', []));
        const isProductionEnv = @json(app()->environment('production'));

        function qzCsrfToken() {
            return document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
        }

        function assertQzTrustedOrigin() {
            if (!qzProductionLock || !isProductionEnv) {
                return;
            }

            const currentOrigin = window.location.origin;
            const allowed = (Array.isArray(qzAllowedOrigins) ? qzAllowedOrigins : [])
                .map(function(item) {
                    try {
            ...
            if (allowed.indexOf(currentOrigin) !== -1) {
                return;
            }

            throw new Error('Origen no autorizado para QZ en producción: ' + currentOrigin);
        }

        async function ensureQzConnection() {
            assertQzTrustedOrigin();
            ...
            if (!qz.websocket.isActive()) {
                qz.security.setCertificatePromise(function(resolve, reject) {
                    ...
                    .then(function(res) {
                        if (!res.ok) {
                            throw new Error('No se pudo obtener el certificado de QZ.');
                        }
                        return res.text();
                    })
                ...
                qz.security.setSignatureAlgorithm(qzSignatureAlgorithm);
                qz.security.setSignaturePromise(function(toSign) {
                    return function(resolve, reject) {
                        ...
                        .then(function(res) {
                            if (!res.ok) {
                                throw new Error('No se pudo firmar la solicitud de QZ.');
                            }
                            return res.json();
                        })
                        .then(function(payload) {
                            if (!payload || !payload.signature) {
                                throw new Error('La firma de QZ no fue válida.');
                            }
        ...
        async function printTicketPdfWithQz(pdfUrl, options = {}) {
            const printerName = options.printerName || qzDefaultPrinterName;
            const copies = options.copies || 1;
            const fallbackOpen = options.fallbackOpen !== false;
            const printMode = (options.printMode || qzPrintMode || 'pixel').toLowerCase();

            if (!pdfUrl) {
                throw new Error('No se recibió la URL del PDF para imprimir.');
            }
            ...
            if (!printer) {
                throw new Error('No hay impresora disponible en QZ Tray.');
            }
            ...
            // QZ no comparte sesión del navegador; enviamos el PDF como base64 autenticado.
            ...
            if (!response.ok) {
                throw new Error('No se pudo descargar el PDF para impresión.');
            }

            const blob = await response.blob();
            const base64Pdf = await new Promise(function(resolve, reject) {
                ...
                reader.onerror = function() {
                    reject(new Error('Error leyendo el PDF para impresión.'));
                };
            ...
            try {
                if (printMode === 'raw') {
                    await qz.print(config, rawPdfData);
                    return { ok: true, printer: printer, mode: 'raw-pdf' };
                }

                // Modo recomendado para ticketera Advance: renderizado PIXEL.
                await qz.print(config, pixelPdfData);
                return { ok: true, printer: printer, mode: 'pixel-pdf' };
            } catch (error) {
                // Si falla el modo elegido, probar el alternativo una vez.
                try {
                    if (printMode === 'raw') {
                        await qz.print(config, pixelPdfData);
                        return { ok: true, printer: printer, mode: 'pixel-pdf' };
        ...
        async function printTicketRawWithQz(saleId, options = {}) {
            const printerName = options.printerName || qzDefaultPrinterName;
            const fallbackOpen = options.fallbackOpen !== false;
            const pdfUrl = options.pdfUrl || null;
            const rawUrl = ticketRawTemplateUrl.replace('__SALE_ID__', saleId);
            ...
            if (!response.ok) {
                throw new Error('No se pudo generar el ticket RAW.');
            }

            const payload = await response.json();
            if (!payload || !payload.text) {
                throw new Error('Contenido de ticket RAW vacío.');
            }

            // ESC/POS: init + center + text + feed + full cut.
            const ticketCommands = '\x1B\x40\x1B\x61\x01'
                + payload.text
                + '\n\n\n'
                + '\x1D\x56\x00';
            ...
            try {
                ...
                return { ok: true, printer: printer, mode: 'raw-escpos' };
            } catch (error) {
                if (fallbackOpen && pdfUrl) {
                    window.open(pdfUrl, '_blank');
                }
                throw error;
            }
    ...
    <script>
        $(document).ready(function() {
            // MODAL OCUTADO; TODOS SON TURNO MAÑANA
    ...
    <script>
        $(function () {
            if ($.fn.select2) {
                $('.select2').select2({
                    width: '100%',
                    theme: 'bootstrap4' // usar solo si incluiste el tema bootstrap4 arriba
                });
            } else {
                console.warn('select2 no se inicializó: plugin no disponible');
            }
        });
    </script>
